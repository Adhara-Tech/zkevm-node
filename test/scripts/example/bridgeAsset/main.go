...
func main() {
	...
	auth.Value = amount
	_, err = bridge.BridgeAsset(auth, 1, auth.From, amount, common.Address{}, true, []byte{})
	chkErr(err)
	...
	getRollupInfoByBlockRange(ctx, client, initBlock.NumberU64(), finalBlock.NumberU64())

	getExitRoots(client)

}

func getExitRoots(client *ethclient.Client) {
...
func getRollupInfoByBlockRange(ctx context.Context, client *ethclient.Client, fromBlock uint64, toBlock uint64) {
	var (
